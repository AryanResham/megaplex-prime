...
export default function FAQ({ data, highlightField }) {
  const d = data || {};
  const items = d.items || [
  ...
  const hl = (field) => highlightField === field ? 'ring-2 ring-green-400 ring-offset-2 rounded-lg transition-all duration-300' : 'transition-all duration-300';
  ...
  return (
    <section className="py-20 bg-gray-50">
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <h2 className={` text-3xl md:text-4xl font-bold text-center text-gray-900 mb-10 ${hl('title')}`}>
          {d.title || 'Frequently Asked Questions'}
        </h2>

        <div className="space-y-3">
          {items.map((item, i) => (
            <div key={i} className={`bg-green-50 rounded-lg overflow-hidden transition-all duration-300 ${highlightField && highlightField.startsWith(`items.${i}`) ? 'ring-2 ring-green-400 ring-offset-2' : ''}`}>
              <button
                onClick={() => toggle(i)}
                className="w-full flex items-center justify-between px-6 py-5 text-left hover:bg-green-100 transition-colors"
              >
                <span className={`font-medium text-gray-800 text-[15px] pr-4 ${hl(`items.${i}.question`)}`}>
                  {item.question}
                </span>
                <span
              ...
              {openIndex === i && (
                <div className={`px-6 pb-5 text-gray-600 text-sm leading-relaxed animate-[fadeIn_0.3s_ease] ${hl(`items.${i}.answer`)}`}>
                  {item.answer}
                </div>
              )}
            </div>
